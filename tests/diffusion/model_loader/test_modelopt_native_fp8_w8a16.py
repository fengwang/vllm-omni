...
"""L1 tests for the FP8-blockwise W8A16 resident checkpoint adapter (P6-S2).

Scenarios: docs/session_2/specs/fp8-w8a16-resident-adapter.md. The on-disk format
mirrors the dequant adapter's (FP8 ``<m>.weight`` + bf16 2D ``._scale`` grid + ``._amax``
twin, root ``quantization_config.json``); this adapter keeps the MLP targets
FP8-resident (renaming ``._scale`` -> ``.weight_scale``) and dequantizes ``lm_head``.
"""
import json
