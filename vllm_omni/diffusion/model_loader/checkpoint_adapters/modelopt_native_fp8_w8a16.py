...
"""ModelOpt-native FP8-blockwise **W8A16 resident** checkpoint adapter (P6-S2 spike).
...
- ``lm_head`` and any other non-target FP8 weight are **dequantized to BF16** (reusing
  ...
  INV-7's BF16-at-compute intent for ``lm_head`` holds;
- BF16 non-targets (attention, norms, projections) pass through unchanged;
- ``._amax`` twins are consumed (informational).

Integrity is fail-fast (INV-6/INV-P5-4), reusing the dequant adapter's
