...
Loads the Phase-5.1 ``nvfp4_blockwise_mixed_v1`` deliverable produced by the
cosmos3-nano-quantization repo. On-disk format, per quantized target module
...
unchanged, so target modules remain FP4-resident (INV-7). Integrity is verified
fail-fast (INV-6): a present-but-invalid sidecar aborts at detection; count /
...
modelopt_native_nvfp4 <model_dir>`` for a GPU-free header + sidecar integrity
...
P5S4_MARKER = "cosmos3-nvfp4-blockwise/p5s4"
...
class CheckpointIntegrityError(ValueError):
    """NVFP4 checkpoint integrity violation (INV-6). Fail fast."""
...
def verify_observations(infos: Iterable[TensorInfo], spec: Nvfp4BlockwiseSpec) -> list[str]:
    """INV-6 structural verification against the sidecar manifest (pure).
...
class ModelOptNativeNvfp4CheckpointAdapter:
    ...
    def validate_source_sidecar(cls, source: object) -> None:
        """INV-6 pre-flight: raise on a present-but-invalid sidecar."""
    ...
    def adapt(
        ...
        logger.info(
            ...
            self._spec.expected_count, P5S4_MARKER,
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if violations:
        ...
        for violation in violations:
            print(f"  {violation}")
        return 1
    print(f"INTEGRITY OK: {model_dir} (marker: {P5S4_MARKER})")
    return 0
