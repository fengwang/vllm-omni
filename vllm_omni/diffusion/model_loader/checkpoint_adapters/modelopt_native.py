...
verified fail-fast (INV-P5-4 of the consuming project): a present-but-invalid
...
P5S2_MARKER = "cosmos3-fp8-blockwise/p5s2"
...
class CheckpointIntegrityError(ValueError):
    """Quantized-checkpoint integrity violation (INV-P5-4). Fail fast."""
...
def verify_observations(
    ...
    """INV-P5-4 stream/header verification (pure).
...
class ModelOptNativeFp8CheckpointAdapter:
    ...
    def adapt(
        ...
        logger.info(
            ...
            "to %s (marker: %s)",
            dequantized, self._spec.n_quantized, self._target_dtype, P5S2_MARKER,
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if violations:
        ...
        for violation in violations:
            print(f"  {violation}")
        return 1
    print(f"INTEGRITY OK: {model_dir} (marker: {P5S2_MARKER})")
    return 0
