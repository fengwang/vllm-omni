...
"""W4A16 NVFP4 config for the Cosmos3 ``nvfp4_blockwise_mixed_v1`` deliverable.

The Session-3 artifact is *weight-only* NVFP4 (packed FP4 weights + FP8 block
scales + FP32 global scale, **no** activation ``input_scale``) and quantizes
*only* the MLP / MLP-MoE-gen projections. Serving it FP4-resident therefore
requires:
...
2. **target-inclusion** layer selection — vLLM's ``ModelOptNvFp4Config`` is
   ...
   it: quantize **only** ``...mlp.{gate,up,down}_proj`` (the sole modules named
   this way in Cosmos3 — both the UND ``language_model.layers.*`` and the GEN
   ``gen_layers.*`` decoder layers), leaving attention, embeddings, ``lm_head``,
...
in a nested ``quantization_config`` block), so the Cosmos3 transformer resolves
...
# The only Cosmos3 modules named ``.mlp.{gate,up,down}_proj`` are the quantized
# MLP targets (both UND ``language_model.layers.N`` and GEN ``gen_layers.N``).
# Attention uses ``to_q``/``to_k``/``to_v``/``to_out``/``add_*_proj``; the small
# ``proj_in``/``proj_out``/``time_embedder``/action/audio layers are plain
# ``nn.Linear`` (never asked for a quant method). So this suffix is exact.
